...
public class StemmerFilter extends TokenFilter {
	...
	public boolean increment() throws TokenizerException {
		...
		while (this.getStream().hasNext()) {
			...
			t.setTermText(s.getStemmedWord(t.getTermText()));
		}
	...
	public static class Stemmer {
		...
		/**
		 * This method adds the char[] into a buffer. And then perform stemming
		 * on the buffer if it passes the input criteria. Finally the stemmed
		 * word will be returned.
		 */
		public String getStemmedWord(String word) {
			boolean hasSpecials = false;
			for(char ch: word.toCharArray()){
				if (!Character.isLetter(ch)) hasSpecials = true; 
			}
			if(hasSpecials){
				return word;
			}else{
				add(word.toCharArray(), word.toCharArray().length);
				stem();
